...
public class ChampagneTower {
    ...
    public static void main(String[] args) {
        ...
        int queryRow = 1;
        int queryGlass = 1;
        double remain = new ChampagneTower().new Solution().champagneTower(poured, queryRow, queryGlass);
        System.out.println(remain);
    }

    /**
     * A top-down dynamic programming solution. We calculate the remaining champagne row by row until reach the target.
     * 
    ...
    class Solution {
        public double champagneTower(int poured, int queryRow, int queryGlass) {
            ...
            for (int r = 0; r < queryRow; r++) {
                for (int c = 0; c <= r; c++) {
                    if (dp[r][c] > 1) {
                        // extra champagne falls to next row
                        dp[r + 1][c] += (dp[r][c] - 1) / 2;
            ...
            return Math.min(dp[queryRow][queryGlass], 1);
        }
    }
